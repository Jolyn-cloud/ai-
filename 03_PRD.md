...
### （六）考点顽固标签【SSOT · 2026-09-02 拍板】

> **口径统一**：顽固只标在**考点级**（= 章节练习三级标题 = 做题页考点 concept），考题、四级不再单独标。

| 项 | 内容 |
|---|---|
| 判定 | 考点内 **错误考题数 ÷ 该考点考题总数 × 100% ≥ 50%** → 该考点顽固 |
| 展示位置 | ① 章节刷题：三级标题（考点）右侧「顽固」红标签<br>② 做题页：考点标签 `📍 考点 · 顽固`（考点标签底色转顽固红） |
| 例题 | 「文件管理」60%（3/5）→ 顽固；「信息与数据」56%（5/9）→ 顽固 |
| 联动 | 顽固考点必进 AI 今日任务并排最前；答题时该考点优先展示错过的考题 |
| 已废除 | ❌ 考题级顽固（答错≥3 次单独标顽固）——不再使用<br>❌ 四级行顽固符号——不再使用 |
