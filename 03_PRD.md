...
**判分与交卷展示（操作题=多道小题单题解析）**

- 合并判分：操作题整体一次判分，子题逐题核对（全对才答对）。
- **交卷 / 背题已答后**：不再分屏，当作单题解析——
  - **上方题目区**：平铺全部子题（题干 + 选项/填空 + 各自答案正误高亮）；
  - **下方解析区**：**逐子题列解析卡片**（小题正误 / 答案 / 你的答案 / 该小题详解），再附**整题总评**（`q.analysis`）+ 解题步骤 + 考点对比/举一反三等通用解析。
- 答题结果行对操作题**不展开长串子题答案**，只显示「回答正确/错误 + 子题答对 N/M」；各子题答案在上方题目区逐题已展示。
