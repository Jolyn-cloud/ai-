...
|---|---|---|---|---|
| 演示外壳（总壳） | 已修复 | `05_UI设计/AI伴学_小程序.html` | §一 / §四（承载） | 功能载体，非独立模块 |
...
| **题库做题模块**（做题页/计时/答题卡/选项标签/反馈） | 原型已落地 | `做题页.html`（总壳 `practiceLayer` 统一承载） | **§六** | [04_功能清单](04_功能清单.md) §三·3 题库页 |
| 逻辑说明面板 | 重构中 | 总壳右侧面板 | — | — |
...
> 总壳（`AI伴学_小程序.html`）、学习页（`登录与画像原型.html`）、我的页（`我的.html`）的登录 · 画像 · 权益行为均以本节为准，代码内不双份维护。
...
| **画像** | 用户个性化资料：**入学年份 + 考试时间 / 专科院校 / 专科专业 /（科目）**。服务个性化（校荐、可报专业、毕业年份、分卷） |
| **画像三态** | **none**（未填）/ **partial**（部分完成）/ **done**（院校+专业+入学年份齐全） |
| **草稿** | 未登录时填写的画像，暂存总壳 `STATE.user`。不随登录/退出清空（PM 决策） |
| **免费 VIP** | 独立权益引导。判据为**领取状态**（`STATE.vipClaimed`）：未领取 → **每次登录/重开进题库页**自动弹（冷启动默认落题库页即触发）；已领取 → 不再弹。领取状态持久化（`zsb_vip_claimed_v1`），首次领取完成即永久不弹 |
...
| `user` | `{nick, year, month, school, major, subject}` | nick 登录时写入；`year`=入学年份(大一/大二/大三/其他)、`month`=考试时间(YYYY-MM-DD)、school/major/subject 画像字段（含草稿） |
| `profileLevel` | `none / partial / done` | 画像三态，`calcProfileLevel(user)` 计算，兼容旧 `profile` 布尔 |
| `vipClaimed` | boolean | 已领取免费 VIP。**持久化**（独立 key `zsb_vip_claimed_v1`，跨刷新/退出保留）：未领取 → 每次登录仍弹；首次领取完成后不再弹 |

#### 1. 画像三态计算

| 条件 | level | 我的页标签 |
|---|---|---|
| 院校 + 专业 + 入学年份（+考试时间，仅「其他」需手选）全填 | `done` | 已完善 |
...
- 权益领取线（benefit）登录成功 → VIP 到账后**停留题库页**（画像已移出冷启动主链路，仅学习页软引导；游客继续体验卷 → 报告 → 解析，见 §七）。

#### 3. 退出（顶栏）
- 清 `logged` + 权益会话锁。
- **保留画像草稿**（`profileLevel` 与 `user` 画像字段保留，PM 决策）。
- **保留 `vipClaimed`**（领取状态持久）：已领取 → 退出再登录不再弹；未领取 → 下次登录仍弹。

#### 4. 登录 / 领取流程图（游客体验卷方案）
> **2026-09-02 以体验卷方案为准重绘**（源文件 `05_UI设计/登录流程_体验卷方案.html`）。历史《登录/领取流程图（三条主线）》`05_UI设计/登录流程_三条主线.html` 已由本方案取代。FigJam 原图：[登录流程-完整版](https://www.figma.com/board/uRSHUftqHKJR4qSRCSZnur)（可编辑）。

![登录/领取流程图（游客体验卷方案）](05_UI设计/登录流程_体验卷方案.png)

**说明（与方案对齐口径）**
- **第一线（冷启动主线 / VIP → 体验卷）**：冷启动闪屏后默认落**题库 tab** → 未领取弹 VIP（仅题库触发）；「关闭（点 X/图外）」与「领取（登录 / VIP 到账）」均**停留题库页** → 游客态显示**免费体验卷（固定 15 题）** → 交卷 → 成绩报告 → 查看解析（已登录直接解析 / 未登录弹登录，见第三线）。
- **第二线（题库页游客态）**：`登录状态 × 已体验标记（zsb_trial_done_v1）` 决定入口——游客未体验显示体验卷卡 + 正式入口锁登录；游客已体验隐藏体验卷卡 + 正式入口锁登录；已登录完整功能无体验卷。体验卷作答存本地会话、不落库不进错题本。
- **第三线（报告 → 解析门禁）**：报告页「查看解析」/答题卡跳题 → 只看 `STATE.logged` → 已登录直接进解析（`ENTER_REVIEW` 定位）；未登录弹登录 Sheet（来源 `review`，记住 `pendingReviewQ`）→ 登录成功直接回跳原题解析，拒绝则停留报告页。
- 详细规则见 **§七 游客体验卷规则**；画像已移出冷启动主链路，仅学习页软引导（见 §四（四））。
...
| 功能 | 位置 | 画像依赖说明 | 引导 |
|---|---|---|---|
| **AI 问答** | 学习页 | 按专业/入学年份个性化回答（校→校荐、专业→可报本科、入学年份→考试时间/毕业年份） | 顶部「补全资料」弱引导（现有） |
...
#### 4. 考试时间口径（系统判断，YYYY-MM-DD）
> **规则**：山东统招专升本考试时间固定在**三年中每年 3 月最后一个周日**，系统按入学年份自动判断，**不靠用户记日期**。

| 入学年份 | 考试时间（3月最后周日） | 说明 |
|---|---|---|
| 大三 | **2027-03-28** | 毕业当年考（统招考试在毕业当年 3 月，最后一次机会） |
| 大二 | **2028-03-26** | 次年考 |
| 大一 | **2029-03-25** | 第三年考 |
| 其他 | 手动选择上述三档之一 | 不自动判断 |

**存储格式**：`STATE.user.month` 存 **`YYYY-MM-DD`**（如 `2027-03-28`），UI/选项/上报所有处一律用纯日期，不用"3月最后周日"中文文案。系统判断规则（每年 3 月最后周日）在代码与文档维护，用户只见具体日期。

### （五）免费 VIP 权益规则

#### 1. 触发（判据=领取状态 + 会话标记，时机=进题库页）
- **是否弹**：看领取状态——`STATE.vipClaimed=false`（未领取）→ 弹；已领取 → 不再弹。**不以「是否首次进入」为判据**。
- **何时弹**：**进题库 tab** 时触发检查；未领取即自动弹（题库/闪卡/我的 中仅题库触发，闪卡/我的不再弹）。**冷启动默认落题库页（闪屏后），未领取即首屏弹出**。
- **会话内只弹一次**：`benefitShown` 置位后不再主动弹；关掉（点X/点图外）→ `benefitClosed`，本会话不再弹。
- **跨会话持续引导**：未领取 → **每次登录/重开仍弹**（`vipClaimed` 持久化，不随刷新/退出清除）；**首次领取完成后不再弹**。

#### 2. 领取（① 已登录直接领取）
- **未登录**：点「领取专属权益」→ 权益卡片缩小消失 → 弹登录 Sheet → 登录成功 → VIP 到账 + Toast「✓ 登录成功，VIP权益已到账」→ **停留题库页**（画像已移出冷启动主链路；游客态体验卷流程见 §七）。
- **已登录**：点「领取专属权益」→ 卡片收起 → **直接 VIP 到账**（不弹登录）+ Toast「✓ 领取成功，VIP权益已到账」→ **停留题库页**。
- 领取路径上**关闭登录 Sheet**（放弃登录）→ 停留题库页；`benefitUsing` 保留（点过领取，后续登录成功 VIP 仍到账）。
- 已领取（`vipClaimed=true`）→ 不再弹权益引导；重复进入走「我的 → VIP 会员」常驻入口。
- **关闭弹窗（点X/图外）** → 本会话不再弹，**停留题库页**（未领取，下次登录仍弹）。
...
| 场景 | 行为 |
|---|---|
| 未登录首次进题库 tab | 弹免费 VIP 权益（会话一次）→ 关闭/领取后停留题库页 |
| 游客 · 未体验 | 显示「免费体验卷」卡（15题）；正式功能入口点击弹登录 |
| 游客 · 已体验（`zsb_trial_done_v1`） | 隐藏体验卷卡；正式功能入口点击弹登录 |
| 未登录点需登录功能 | 弹登录 Sheet（登录归属总壳） |
| 需画像功能（B 档）未完成画像 | 软引导（提示 + 可跳过）→ 跳首页（学习页 tab）+ 弹画像 Sheet |
| 报告页点「查看解析」· 未登录 | 弹登录 Sheet（来源 `review`）→ 登录成功直接回跳解析 |
...
> 本章节固化做题页（`做题页.html`，总壳 `practiceLayer` 统一承载题库/错题本/章节练习入口）的核心规则。
> 涉及数据层字段为 **v5.0 接入时契约**，演示期手写 mock。

### （一）做题页结构与交互

| 区块 | 规则 |
|---|---|
| 顶部导航 | **模式切换（左）+ 答题计时（中）+ 答题卡入口（右）**；内部 ✕ 退出按钮已删除，统一走总壳 appbar 的「退出」返回上一级 |
| 进度条 | 左 = 第 X 题；右 = `Y/Z · 共 N 题`（原顶部 nav-progress 下沉至此） |
| 模式 | exam（做题）= 全部答完交卷统一显示答案解析；study（背题）= 每答一题立即显示解析 |
| 翻题 | 右→左下一题 / 左→右上一题（手势）；**操作/分析/综合运用题为分屏模式，不响应左右滑翻题**（见（一.1）） |
| 底部栏 | 收藏 / 标记 / 分享 + 交卷（背题模式/交卷后隐藏交卷） |

### （一.0）考试卷：模拟机考形态【SSOT · 2026-09-02 拍板】

> 模拟考场（`entry=exam`）/ 历年真题（`entry=real`）为**模拟机考形态**；体验卷（`trial`）与练习类（今日学习/错题/章节）不在此列，保留原逻辑。

| 区块 | 规则 |
|---|---|
| 模式 | **无背题模式**——隐藏「做题/背题」切换，`MODE` 锁定 `exam`；背题仅保留给练习/错题/章节/体验卷 |
| 倒计时 | **统一限时 120 分钟**（`PAPERS` 中 exam/real 各卷 `minutes:120`），`navTimer` 显示剩余 `MM:SS`，到 0 自动交卷；trial 保持 15 分钟不变 |
| 进度条 | **= 当前题型内进度**：左 = `题型名 第 X 题`（如「单项选择题 第 2 题」），右 = `offset+1 / 本大题题数`，进度条宽度 = `(题内序号+1)/本大题题数`；做到下一题型自动切换（单选做完 → 多选 1/N…）。题目须按题型分大题连续排列（单选→多选→判断→填空→操作）。非考试卷保持原整卷进度逻辑 |
| 答题卡 | **按题型分区**：每段前插规则块（中文大题号「一、二、三…」+ `本大题共 N 小题，每小题 X 分，共 Y 分`，分值一致才显「每小题 X 分」；判断题段加补充说明）；格子按卷内绝对题号连续，点击跳题不变。非考试卷答题卡保持原平铺 |

### （一.1）操作/分析/综合运用题：分屏模式【SSOT · 2026-09-02 拍板】

> 一道操作题 = **一个固定题干 + 多个子题**。用户在分屏上边读题干、边在下方完成子题，不是「一题一页」，题干**不做弹窗、不遮罩**。

**屏幕结构（上中下三段）**

| 区块 | 规则 |
|---|---|
| ① 题干阅读区（上） | 默认显示完整题干，**无遮罩、默认不隐藏**；默认占比 **40%**；**独立滚动**（题干长时仅在区内滑动，不联动下方） |
| ② 中间可拖拽分界线 | 明显但轻量的拖拽手柄（≡）；上下拖动调整两区高度；**题干区最小 25% / 最大 65%**，钳制避免某一区域被完全挤压 |
| ③ 子题作答区（下） | **顶部横向小题导航** + 当前子题内容；左右可横向滑动导航（子题多时） |

**横向小题导航（子题切换的唯一入口）**

- `第 1 题 第 2 题 第 3 题 …`，**当前小题高亮**（主色底）。
- **已答子题**显示完成态 `✓`（tab 尾部加 ✓）；答案保留在 `answers[idx][subIdx]`。
- 点击某 tab → 仅切换下方当前子题，**题干区保持不变**。
- **作答区底部不设「上一题 / 下一题 / 完成操作题」按钮**（2026-09-02 PM）；子题切换只用点击 tab。

**子题作答**

- 子题可为单选/判断/填空，用同一个分屏结构，只替换下方作答组件（选项 / 输入框）。
- **切走再切回不丢失已填答案**（答案存对象 `answers[idx][subIdx]`，非临时态）。

**整题进退**

- 与普通题一致：**左右滑手势翻整题**（操作题不拦截横向手势；分屏内滚动是纵向的，与横向翻题不冲突）。

**判分与交卷展示（操作题=分屏单题解析，2026-09-02 拍板）**

- 合并判分：操作题整体一次判分，子题逐题核对（全对才答对）。
- **交卷 / 背题已答后**：**仍走分屏**，与作答态同结构（题干区 + 分界线 + 子题作答区 + 横向 tab）；区别仅——
  - 子题作答组件带对错着色（选项/填空标对错色）、输入禁用；
  - tab 标**答对✓ / 答错✗ / 未答—**（替代作答态的「已答✓」）；
  - 分屏**不占满**、分界线隐藏不可拖、题干区与作答区取自然高度，把空间让给下方解析区；
  - **下方解析区**：只渲染**当前 tab 选中子题**的解析卡片（小题正误 / 答案 / 你的答案 / 该子题详解），tab 切换子题时与题目区同步切换；**不再平铺全部子题**。
- **无整题总评**：`q.analysis`（整题考点串联）不再展示——各子题详解已覆盖考点，避免重复；数据层字段保留不删。
- 答题结果行对操作题**不展开长串子题答案**，只显示「回答正确/错误 + 子题答对 N/M」。

**已废弃（2026-09-02）**

- 旧交互「题干页上滑进小题 / 小题页左滑下一子题 / 下滑收起浮窗」**已删除**；旧分屏底部**上一题/下一题/完成操作题按钮**随后亦删除。
- 分屏下子题切换只用点击横向 tab；整题进退用左右滑手势。

### （二）答题计时（会话级）

- **起算**：进入做题页即起表（`startAt = Date.now()`），切换做题/背题模式**不清零**。
- **显示**：`MM:SS`（等宽数字防跳动）；交卷即定格（`stopTimer`）。
- **记录**：交卷时 `finalSec` 作为「本次做题耗时」字段，结果页 stats「用时」行同步展示；**v5.0 接数据层时一并上报**。

### （三）答题卡（上滑遮罩浮窗）

- **入口**：顶部右侧「答题卡」按钮，点击上滑浮窗。
- **三态统计**（图例）：已答（蓝实心）/ 未答（白底灰边）/ 已标记（橙点角标，可与已答叠加）。
- **题号格**：当前题蓝圈高亮；**点击题号跳转**（`jumpTo` → 更新 idx + renderQuestion + 关闭浮窗）。
- **实时性**：浮窗打开期间，答题/标记/切模式时自动重渲染（图例计数与格状态同步）。

### （四）选项标签：易错 / 易漏选【埋点统计口径 · 暂定 30%】

> **数据来源**：学生做题埋点统计（v5.0 接埋点平台后按下列口径计算；演示期手写 `q.hotWrong` / `q.hotMiss` 模拟）。

| 标签 | 定义 | 颜色 | 位置 |
|---|---|---|---|
| **易错** | 该选项为**错选项** + `错选次数 / 总答次数 ≥ 30%` → `hotWrong = 该下标` | 红 `#EF4444`（刺眼） | 选项卡片最右端 |
| **易漏选** | 该选项为**正确项** + `漏选次数 / 总答次数 ≥ 30%` → `hotMiss = [下标数组]` | 橙 `#F28B3D`（醒目） | 选项卡片最右端 |

**规则**：
- **显示时机**：仅**答案出现后**显示（做题模式交卷后 / 背题模式已答后）；**答题期间不显示**，避免泄露正误。
- **互斥**：同一选项最多一个标签（错选项不可能是正确项，天然互斥）；优先级 **易错 > 易漏选**。
- **范围**：所有选择题（单选/多选/判断）；填空/简答/操作题无选项，不打。
- **阈值**：暂定 **≥ 30%**（PM 拍板，后续可调）；**总答次数**口径 = 该题被作答过的总人次（不含未答）。

### （五）题目反馈（同步后台管理端）

- **入口**：选项下方居中，不显眼字体（`#98A0B2` 11px 下划线）「题目有错误？点击反馈」。
- **弹窗内容**：
  - 错误类型（**多选** chips）——**两组分别**：
    - **题目反馈**：题干错误 / 选项错误 / 答案错误 / 题型错误
    - **解析反馈**：答案解析错误 / 文字解析错误 / 图片解析错误
  - 补充说明（选填 textarea，200 字内）
- **提交**：Toast「已反馈，谢谢」+ 关闭弹窗。
- **数据同步**：payload `{ type:'QUESTION_FEEDBACK', questionId, entry, chapter, concept, errorTypes:[], detail, ts }`，演示期 `console.log` + `postMessage` 转发总壳；**v5.0 接后台反馈管理模块**。

### （六）考点薄弱与顽固标签【SSOT · 2026-09-04 拍板】

> **口径统一**：薄弱与顽固都判在**三级考点**层（= 章节练习三级标题 = 做题页考点 concept），考题、四级不再单独标。两个口径分母统一为「答过考题数」，形式对齐。
> **层级关系**：一级考点（章）→ 二级考点 → **三级考点（判定层 + 首页今日任务展示层）** → 四级考点 → 具体考题。

**判定口径（三级考点层）：**

| 类型 | 判断口径 | 触发条件 |
|---|---|---|
| 薄弱 | 三级考点内 **错误考题数 ÷ 答过考题数 ≥ 30%** 且 **答题样本 ≥ 1** | 答 1 题错 → 100%≥30% → 薄弱 |
| 顽固 | 三级考点内 **错误考题数 ÷ 答过考题数 ≥ 50%** 且 **错误数 ≥ 2** | 答 1 题错 → 只薄弱不顽固；答错 2/3=67% → 顽固 |
| 包含关系 | 顽固是薄弱中更严重的一档（50% ≥ 30%，触发顽固必同时满足薄弱） | — |

> **冷启动防误判**：顽固加「错误数 ≥ 2」门槛，防止只答 1 题错（错误率 100%）就误判顽固。薄弱降门槛到「样本 ≥ 1」，保证首次答题即可判定。
...
|---|---|---|
| 章节刷题三级标题 | — | 「顽固」红标签 |
| 做题页考点标签 | — | `📍 考点 · 顽固`（底色转顽固红） |
| 今日任务卡 | 顽固/薄弱三级考点 + 错误率 | 顽固优先 |

**AI 今日任务组题规则（首页展示二级考点；组题范围 = 选定二级下所有三级考点的考题）：**

选定二级考点（含顽固三级的优先 → 含薄弱三级的 → 按考纲顺序），在其下所有三级考点间跨级取题：

| 层 | 取题来源 | 范围 |
|---|---|---|
| 1 | 顽固**三级**考点的错题 | 选定二级内 |
| 2 | 一般薄弱**三级**考点的错题 | 选定二级内 |
| 3 | 上述**三级**考点的未答考题 | 选定二级内 |

- 目标 **35 道**，逐层下探；三层取完仍不足 35 → **收缩到实际可用题量**（不跨二级补齐）
- 做题页内：同三级考点错题优先展示（展示顺序，与组题范围分开）
- 首页标题：展示选定的**二级考点**（不展示三级/四级）
- 冷启动无薄弱项 → 按考纲顺序选首个二级，走第 3 层出未答考题，标签「入门」、理由「首次学习，从基础开始建立摸底」

**例题：**
- 「文件管理」答过 5 题、错 3 题 → 错误率 60% ≥ 50% 且错误数 3≥2 → 顽固
- 「进程与线程」答过 3 题、错 1 题 → 错误率 33% ≥ 30% 但 < 50% → 薄弱
- 「信息与数据」答过 1 题、错 1 题 → 错误率 100% ≥ 30% → 薄弱；错误数 1 < 2 → 不顽固

**已废除：**
- ❌ 考题级顽固（答错≥3 次单独标顽固）——不再使用
- ❌ 四级行顽固符号——不再使用
- ❌ 薄弱「正确率 < 60% 且 答题样本 ≥ 5」——2026-09-04 改为错误率口径降门槛

**相关不动的部分**（独立体系）：
- 闪卡「顽固卡」（未记住≥3，记忆口径）——两套体系，互不影响。
- 错题本「顽固错题」手动分类与「顽固重练」筛选（插入题打标，非自动考点级）。

---

## 七、游客体验卷规则【新方案 · SSOT】

> **定位**：未登录用户（游客）的冷启动体验入口——用一套固定 15 题的免费体验卷让游客零门槛走完「做题 → 出分 → 看解析」核心闭环，在**高意图时刻（想看解析）**引导登录，登录成功后直接回跳解析页兑现体验。
> **关联**：[04_功能清单](04_功能清单.md) §3 题库页 · §5 个人中心 ｜ 流程图：[登录流程_体验卷方案.html](05_UI设计/登录流程_体验卷方案.html)
> **已拍板**：① 画像移出冷启动主链路（仅学习页软引导）② 体验卷入口只对游客显示 ③ 体验过后游客态展示正式功能入口（点击弹登录），不重复推体验卷。

### （一）体验卷定义

| 项 | 内容 |
|---|---|
| 命名 | 「免费体验卷」（界面不出现"测试/模拟"字样，避免与正式考试混淆） |
| 题量 | **固定 15 题**（一套，不随机出题） |
| 科目 | 计算机基础（默认科目） |
| 题型配比 | 单选×7 / 多选×3 / 判断×3 / 填空×2（每题含 `analysis`/`steps` 解析） |
| 满分 / 时长 | 100 分 / 15 分钟倒计时（约 5 分钟可完成） |
| 载体 | 做题页 `entry=trial`（`PAPERS.trial1`，同考试模式：交卷 → 成绩报告） |

### （二）展示规则（题库页游客态）

| 状态 | 题库页表现 |
|---|---|
| 游客 · 未体验 | 显示「免费体验卷」高亮卡（15题 · 免登录 · CTA「开始体验」）；正式功能入口（章节/模拟/真题/错题/快捷行）点击 → 弹登录 |
| 游客 · 已体验（`zsb_trial_done_v1=1`） | 隐藏体验卷卡；正式功能入口点击 → 弹登录 |
| 已登录 | 完整功能，无体验卷入口 |

- **已体验标记**：`zsb_trial_done_v1` 持久化（与 `vipClaimed` 同款独立 key，不随刷新/退出清除）。游客完成体验卷交卷（总壳收到 `REQ_REPORT` 且 `type='trial'`）时写入。
- **「完善资料」（画像）不设登录门禁**：未登录可填画像草稿（与 §四（四）一致）。

### （三）游客数据策略

- 游客作答/成绩**存本地会话，不落库、不进错题本/刷题记录**；登录后仅用于本次解析展示。
- **v5.0** 接数据层时再定游客数据合并策略（游客 token / 登录后合并）。

### （四）解析门禁（review 登录来源）

- **判据**：只看 `STATE.logged`，与 `vipClaimed` 无关。
- 报告页「查看解析」/答题卡跳题（`REPORT_REVIEW`）→ 未登录 → 总壳 `openLoginSheet('review')` + 记录 `pendingReviewQ`；登录成功 → 复用 `REPORT_REVIEW → ENTER_REVIEW` 链路**直接回跳原题解析**；拒绝登录 → 停留报告页（可再次作答 / 稍后登录）。

### （五）流程分支（总览）

| 分支 | 路径 |
|---|---|
| ① 未登录 · 关掉 VIP | 题库tab → VIP弹窗 → 关闭 → 停留题库页 → 体验卷 → 交卷 → 报告 → 查看解析 → 弹登录 → 登录成功 → 解析 |
| ② 未登录 · 领取 VIP | VIP弹窗 → 领取 → 登录Sheet → 登录成功 → VIP到账 → 停留题库页 → 体验卷 → 报告 → 解析（免登） |
| ③ 已登录 · 领取 VIP | VIP弹窗 → 领取 → VIP直接到账 → 停留题库页 → 体验卷 → 报告 → 解析（免登） |
| ④ 领取后 · 登录时拒绝 | VIP弹窗 → 领取 → 登录Sheet 关闭 → 草稿保留 → 停留题库页 → 体验卷 → 报告 → 查看解析 → 再弹登录 |

### （六）与冷启动主链路的关系

- 关闭/领取 VIP 后均**停留题库页**（不再跳学习页）；冷启动链路变为：`闪屏 → 题库tab → VIP弹窗 → 体验卷 → 报告 → 登录 → 解析`。
- **画像已移出冷启动主链路**：不再作为冷启动必经步骤；画像采集仅学习页软引导（§四（四）2），与登录状态互相独立。
...
| 2026-08-30 | **登录流程优化**：冷启动默认落**题库 tab**（闪屏后进入，学习页改懒加载）；未领取 VIP 自动弹 → 领取（登录 → VIP 到账 → Toast）或关闭（点X/图外）→ **跳转学习页填画像 → 登录**；`vipClaimed` **持久化**（独立 key `zsb_vip_claimed_v1`，不随刷新/退出清除）：未领取 → 每次登录/重开仍弹，首次领取完成后不再弹 | PM 拍板：闪屏后 → 题库页领取 VIP（弹窗）→ 领取/关闭 → 跳转 AI 页 → 填写用户画像 → 登录；未领取的每次登录仍显示领取，直到首次领取完成后不显示弹窗 |
| 2026-08-30 | **流程图/文档向代码对齐**：重绘《登录/领取流程图（三条主线）》（源文件 `登录流程_三条主线.html` → 截图 `登录流程.jpg`）；第一线触发时机明确「首次进题库 tab（仅题库触发，闪卡/我的不再弹）」+ 补画「关闭（点X/图外）→ 跳学习页填画像」分支；第二线明确「不填 → AI 正常对话、填 → 是否已登录」；总壳文件名引用统一为 `AI伴学_小程序.html`；总壳逻辑说明 desc 改为「冷启动落地（登录引导入口）」 | PM 拍板：以当前代码为准改流程图与文档，不动代码逻辑 |
| 2026-08-31 | **新增 §六 题库做题规则**：做题页结构/交互（顶部三栏 + 进度条）、答题计时（会话级 + finalSec 上报）、答题卡（上滑浮窗 + 三态 + 点题号跳转 + 实时刷新）、选项标签（易错红/易漏选橙，互斥，**埋点阈值暂定 ≥30%**）、题目反馈（错误类型多选 + 补充说明 + 同步后台管理端）；原 §六变更记录顺延为 §七 | PM 拍板：埋点阈值暂定 30%；错误类型多选；反馈同步后台；做题页内部退出按钮删除，统一走总壳 appbar |
| **2026-09-02** | **新增 §七 游客体验卷规则 + 重绘登录流程图**：冷启动链路重构为「题库tab → VIP弹窗 → **停留题库页** → 游客体验卷（固定15题）→ 交卷 → 成绩报告 → 查看解析（登录门禁 `review` 来源）→ 登录成功直接回跳解析」；关闭/领取 VIP 后**停留题库页**（不再跳学习页）；**画像移出冷启动主链路**（仅学习页软引导）；体验卷入口**仅游客显示** + `zsb_trial_done_v1` 已体验标记（体验后游客态展示正式功能入口，点击弹登录）；游客作答不落库不进错题本；流程图源文件 `登录流程_三条主线.html` 替换为 `登录流程_体验卷方案.html`；原 §七变更记录顺延为 §八 | PM 拍板：① 画像不再出现在冷启动主链路 ② 体验卷入口只对游客显示 ③ 体验过后游客态改为展示正式功能入口（点击弹登录） |
| **2026-09-02** | **题目/解析反馈类型重构**：题目反馈 **4 类**（题干错误 / 选项错误 / 答案错误 / 题型错误）+ 解析反馈 **3 类**（答案解析错误 / 文字解析错误 / 图片解析错误），替换旧 6+5 类；PRD §六（五）与做题页 `fbOptsQ/fbOptsA` 同步 | PM 拍板：反馈弹窗类型按题目组 / 解析组精简 |
| **2026-09-02** | **操作题分屏模式**（做题页）：操作/分析/综合运用题改为「上题干区 + 中可拖拽分界线 + 下子题作答区」三段分屏；题干区默认 40%（可拖 25%~65%）、独立滚动、不遮罩；下方横向小题导航（当前高亮、已答标✓、点击切换不丢答案）+ 底部上一题/下一题，最后子题显示「完成操作题」→ 全部子题答完才进下一道普通题；**删除旧遮罩浮窗 + 上滑/左滑手势**；交卷后仍纵向平铺全部子题看解析。PRD 新增 §五（一.1）；数据层 today/卷内操作题演示子题扩为 5 个 | PM 拍板：分屏+拖拽+横向导航；交卷后保留全览平铺；删除旧手势 |
| **2026-09-02** | **考试卷模拟机考形态**（做题页）：模拟考场/历年真题卷改为模拟机考——**无背题模式**（隐藏做题/背题切换，MODE 锁 exam）、**倒计时统一 120 分钟**（`PAPERS` exam/real 各卷 `minutes:120`，trial 保持 15）、**进度条按题型内推进**（左=题型名+第X题，右=题内序号/本大题数，宽度按题型内进度；题目须按题型分大题连续排列）、**答题卡按题型分区**（规则块=中文大题号+本大题共N小题每小题X分共Y分+判断题补充，分值一致才显每小题X分；格子按卷内绝对题号连续）。PRD 新增 §六（一.0）；exam/real 卷 mock 数据按题型重排（单选→多选→判断→填空→操作） | PM 拍板：考试卷=模拟机考（无背题/120分钟/题型内进度/答题卡题型分区）；mock 数据按题型分大题重排 |