...
| ③ 子题作答区（下） | **顶部横向小题导航** + 当前子题内容 + 底部上一题/下一题按钮；左右也可横向滑动导航（子题多时） |
...
- 上一题 / 下一题；**最后一个子题时按钮变为「完成操作题」**。
- 「完成操作题」仅当**全部子题已作答**才前进到下一道普通题；否则 Toast 提示「请完成全部子题后再继续」。

**判分与交卷展示**

- 合并判分：操作题整体一次判分，子题逐题核对（全对才答对）。
- **交卷 / 背题已答后**：不再分屏，纵向平铺全部子题（含答案正误 + 各自解析），便于对答案看解析。

**已废弃（2026-09-02）**

- 旧交互「题干页上滑进小题 / 小题页左滑下一子题 / 下滑收起浮窗」**已删除**。分屏下仅用拖拽分界线 + 点击导航 + 底部按钮切换；左右滑手势在操作题上不生效（避免与导航横向滚动冲突）。
