...
- 权益领取线（benefit）登录成功 → VIP 到账后**跳转学习页**（未画像填画像 / 已画像进正常首页）。
...
#### 4. 登录 / 领取流程图（三条主线）
> 2026-08-29 与 PM 口述对齐后确认；**2026-08-30 以当前总壳实现为准重绘**（源文件 `05_UI设计/登录流程_三条主线.html`，截图 `登录流程.jpg`）。FigJam 原图：[登录流程-完整版](https://www.figma.com/board/uRSHUftqHKJR4qSRCSZnur)（可编辑）。

![登录/领取流程图（三条主线）](05_UI设计/登录流程.jpg)

**说明（与代码对齐口径）**
- 三条线入口独立，收尾统一：登录成功 → `broadcast()` 状态全局同步（所有页面一致）。
- **第一线（功能 tab / VIP 领取）**：触发时机 = **首次进入题库 tab**（冷启动闪屏后默认落题库页；仅题库触发，闪卡/我的不再弹）；「关闭（点 X/图外）」分支 → 本会话不再弹 → **跳学习页填画像**（未领取，下次登录仍弹）。
- **第二线（AI 首次互动 / 画像）**：**不填 → AI 正常对话**（游客可试用、不强制登录）；填 → 是否已登录 → 未登录走登录（拒绝登录 → 草稿保留，不再强弹）。
- **第三线（我的页 / 需账号功能）**：未登录 → 总壳登录 Sheet → 登录成功。
- 红线未罗列的细节（协议勾选、拒绝登录保留草稿、领取后跳学习页、未领取每次登录仍弹等）见本节四（三）1 / 四（四）/ 四（五）规则。
...
#### 2. 领取（① 已登录直接领取）
- **未登录**：点「领取专属权益」→ 权益卡片缩小消失 → 弹登录 Sheet → 登录成功 → VIP 到账 + Toast「✓ 登录成功，VIP权益已到账」→ **跳转学习页**（未画像填画像 / 已画像进正常首页）。
- **已登录**：点「领取专属权益」→ 卡片收起 → **直接 VIP 到账**（不弹登录）+ Toast「✓ 领取成功，VIP权益已到账」→ **跳转学习页**。
- 领取路径上**关闭登录 Sheet**（放弃登录）→ 跳学习页填画像；`benefitUsing` 保留（点过领取，后续登录成功 VIP 仍到账）。
- 已领取（`vipClaimed=true`）→ 不再弹权益引导；重复进入走「我的 → VIP 会员」常驻入口。
- **关闭弹窗（点X/图外）** → 本会话不再弹，**跳转学习页填画像**（未领取，下次登录仍弹）。
