...
> **口径统一**：薄弱与顽固都标在**考点级**（= 章节练习三级标题 = 做题页考点 concept），考题、四级不再单独标。两个口径分母统一为「答过考题数」，形式对齐。

**判定口径：**

| 类型 | 判断口径 | 触发条件 |
|---|---|---|
| 薄弱 | 考点内 **错误考题数 ÷ 答过考题数 ≥ 30%** 且 **答题样本 ≥ 1** | 答 1 题错 → 100%≥30% → 薄弱 |
| 顽固 | 考点内 **错误考题数 ÷ 答过考题数 ≥ 50%** 且 **错误数 ≥ 2** | 答 1 题错 → 只薄弱不顽固；答错 2/3=67% → 顽固 |
...
| 今日任务卡 | 薄弱考点 + 错误率 | 顽固优先排最前 |

**AI 今日任务组题规则（按优先级分层取题，逐层兜底到 35 题）：**

| 层 | 取题来源 | 说明 |
|---|---|---|
| 1 | 顽固考点里的**错题** | 优先抽完 |
| 2 | 一般薄弱考点里的**错题** | 顽固错题不够 35 时补 |
| 3 | 薄弱考点里的**未答考题** | 错题抽完仍不够时，从同考点补 |
| 4 | 按**考纲顺序**的未学考点题 | 全部薄弱考点都不够时，从第 1 章顺序补 |
| 空态兜底 | 无任何薄弱（冷启动首刷） | 直接走第 4 层：从第 1 章顺序出 35 道入门卷 |

- 固定 **35 道**，不足逐层下探到 35
- 做题页内：同考点错题优先展示（展示顺序，与组题范围分开）
- 冷启动无薄弱项 → 走第 4 层兜底，标签「入门」、理由「首次学习，从基础开始建立摸底」
