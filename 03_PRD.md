...
| **免费 VIP** | 独立权益引导。判据为**未领取**（`STATE.vipClaimed=false`）：未领过 → **首次进题库 tab** 自动弹；已领取 → 不再弹。触发时机是「首次进题库页」，是否展示看领取状态 |
...
#### 1. 触发（判据=领取状态 + 会话标记，时机=首次进题库页）
- **是否弹**：看领取状态——`STATE.vipClaimed=false`（未领取）→ 弹；已领取 → 不再弹。**不以「是否首次进入」为判据**。
- **何时弹**：**首次进题库 tab** 时触发检查；未领取即自动弹（题库/闪卡/我的 中仅题库触发，闪卡/我的不再弹）。
- **会话内只弹一次**：`benefitShown` 置位后不再主动弹；关掉（点X/点图外）→ `benefitClosed`，本会话不再弹。
- **刷新/重开** 且未领取 → 再次显示（总壳 `STATE` 完全冷启动，登录/画像/VIP 全重置）。
